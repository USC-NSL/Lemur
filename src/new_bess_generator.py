...
from nfcp_bess_generator import new_setup_pipeline, class_and_arg, new_gate_add, new_branch_add
...
def def_instance(module):
    instance_def = ("%s = %s" % (module.name, class_and_arg(module)))
    return instance_def

def nsh_connection(module, gate_index, nic_name):
    connection = ("%s_nsh:%d -> %s" % (nic_name, gate_index, module.name))
    return connection
...
def nic_pipeline(list_of_subgroup, nfcp_parser, nic_name):
    ...
    for subgroup in copy_list_of_subgroup:
        ...
        for module in subgroup:
            return_str.append(def_instance(module))

        # define nsh gate
        for module in subgroup:
            if module.bounce:
                return_str.append(new_gate_add(module, gate_index,nic_name))
#                return_str.append(nsh_connection(module, gate_index, nic_name))
...
#            queue_index += 1
            concate_str = ""
            for module in subgroup:
                concate_str += (" -> %s" % class_and_arg(module))
            ...
            return_str.append("%s_nsh:%d -> q%d -> %s" % (nic_name, root_module.nsh_gate, queue_index, root_module.name))
            return_str.append('q%d.attach_task(wid=%d)' % (queue_index, queue_index))
            queue_index += 1
            output_str = ""
            pop_queue = []
            ...
            while len(subgroup) >0:
                ...
                if len(cur_module.adj_nodes)>1:
                    bpf_def, output_str, transit_condition, _ = new_branch_add(cur_module, bpf_index, output_str)
                    ...
                    if cur_module.adj_nodes[0].is_p4():
