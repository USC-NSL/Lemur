"""
* The script implements an abstract ingress_match_action class that
* parses the ingress action/table specification part in a Lemur P4
* module.
...
def extract_keyword_from_string(target_str, target_mark):
    """ This function extracts the key word, that is wrapped around
    by two 'target_mark' marks.
    For example:
    Input: target_str = "  #silkroad# "; target_mark = '#'
    Output: output_str = "silkroad"
    """
    if target_str.count(target_mark) != 2:
        return ""
    str_len = len(target_str)
    s_index = -1
    e_index = str_len
    for i in range(str_len):
        if target_str[i] == target_mark:
            s_index = i
            break
    for i in reversed(range(str_len)):
        if target_str[i] == target_mark:
            e_index = i
            break
    return target_str[s_index+1:e_index]

class p4_action(object):
    """ The p4_action class represents a p4 action.
...
class p4_table(object):
    """ p4_table class represents a p4 table.
    This class can parse a P4lib file for P4 tables. It also generates
    ...
    def update_default_action(self, default_action_name):
        """ This function sets up the default action for the table. The
        default action must be included in the table.action_set.
        """
    ...
    def generate_apply_p4_code(self):
        """ This function generates the table apply code for the 'apply{...}'
    ...
    def generate_apply_p414_code(self):
        """ This function generates the table apply code for the control block
        """
        res_code = "\tapply(%s);\n" %(self.table_name)
        return res_code
...
class ingress_match_action:
    ''' The class reads the P4 lib to get actions, tables, and the apply
    logic (i.e. the ingress pipeline). Call 'generate_p4_code' or
    'generate_p414_code' to generate code for the actions, tables and
    apply logic.
    '''
    ...
    def read_default_prefix(self, filename):
        """ The function reads the default prefix setting
    ...
    def read_field_operations(self, filename):
        """ The function reads all structs including field_list and 
    ...
    def read_ingress(self, filename):
        """  The function reads actions, tables, and apply logic for
        an ingress control block (i.e. an ingress pipeline).
        """ 
        tmp_str = ""
        with open(filename, 'r') as fp:
            line_count = 0
    ...
    def read_actions(self, filename):
        """ This function reads all actions in the library file
        e.g.
        action drop() {
            mark_to_drop();
        }
    ...
    def read_tables(self, filename):
        """ This function reads all tables in the library file.
        See examples:
        // P4-16 table
        table ipv4_lpm {
            key = {
    ...
    def read_apply_rule(self, filename):
        """ This function parses the ingress apply logic (i.e. a P4 pipeline).
        It spilts the apply logic into two parts, i.e. 
        (1) init_metadata()
    ...
    def preprocess_p4_action_code(self, action_name, action_str, lang_version):
        """ The function pre-processes an action with name 'action_name'.
    ...
    def preprocess_p4_table_code(self, table_name, table_str, lang_version):
        """ It generates the final P4 code for the "table_name" table.
        Solution 1: 
    ...
    def generate_p4_apply_rule_code(self):
        """ It generates the P4-16 code for apply logic. To avoid naming
        conflicts, the function has to add the prefix to actions and tables
        respectively.
        """
        self.p4_apply_code = ""
    ...
    def generate_p414_apply_rule_code(self):
        """ This function generates the P4-14 code for apply logic.
        To avoid naming conflicts, it has to add the prefix to tables
        respectively.
    ...
    def generate_p4_default_code(self):
        """ This function generates the default code for a myIngress
        function by updating the self.p4_code.
        """
        if self.p4_code == None:
            self.p4_code = ""
